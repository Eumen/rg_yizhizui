...
function fs_table_field($id,$table,$field="name",$default='无'){
	if(!empty($id)){
		$field =  M($table)->where(array('id'=>$id))->getField($field);
		return $field;
	}else{
		return $default;
	}
...
/**@see 用户等级 **/
function fs_user_rand($rand){
	$field =  M('rand_price')->where(array('rank_mark'=>$rand))->getField('name');
	return $field;
}

/**@see 用户真实姓名 **/
function fs_user_info($user_id,$field){
	if(!empty($user_id)){
		$field =  M('user_infos')->where(array('user_id'=>$user_id))->getField($field);
		if(empty($field)){ return '无'; }
		return $field;
	}else{
		return '无';
	}
...
/**@see 用户激活时间 **/
function fs_user_audit_time($audit_time){
	 return !empty($audit_time) ? '未激活' : $audit_time;
}

/**@see 用户激活状态 **/
function fs_user_status($user_status){
	 return $user_status == 0 ? '未激活' : '已激活';
}
...
function fs_msg_status($msg_id){
	...
	if($mstatus==0){
		...
		return "<font style='color:green;'>已回复</font>,<font style='color:red;'>未查看</font>";
	}else if($mstatus==2){
		return "<font style='color:#ccc;'>已回复,已查看</font>";
	}
}

function fs_order_status($status){
	return M('order_status')->where(array('status_number'=>$status))->getField('status_name');
}


function fs_region_tongji_money($area_id=-1){
	if(!intval($area_id)) return;
	$money =  M('orders')->where(array('pay_status'=>1, '_string'=>'province='.$area_id.' || city='.$area_id.' || district='.$area_id))->sum('price');
	return $money>0?$money:0;
}
function fs_region_tongji_num($area_id=-1){
	if(!intval($area_id)) return;
	$num =  M('orders')->where(array('pay_status'=>1, '_string'=>'province='.$area_id.' || city='.$area_id.' || district='.$area_id))->count('id');
	return $num>0?$num:0;
}

function user_rand($unit){
	return M('rand_price')->where(array('rank_mark'=>$unit))->getField('name');
}
