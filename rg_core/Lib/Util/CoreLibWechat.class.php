...
class CoreLibWechat
{
	...
	*/
	...
	function rg_http_post($url,$param){
		$oCurl = curl_init();
		if(stripos($url,"https://")!==FALSE){
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
			curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
	 ...
	 */
	public function checkAuth($appid='',$appsecret='',$token='',$rg_start=1){
		if (!$appid || !$appsecret) {
			$appid = $this->appid;
		...
		if($rg_start){
			$cache_token	= include "data/rg_cache/cache_token_".md5($appid."-".$appsecret).".php";
			if( $cache_token['access_token'] ){
				$this->access_token = $cache_token['access_token'];
				return $this->access_token;
			}
		}
	...
	public function createMenu($data, $matchrule=null){
	...
	public function uploadMedia($data, $type){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->rg_http_post(self::UPLOAD_MEDIA_URL.self::MEDIA_UPLOAD.'access_token='.$this->access_token.'&type='.$type, $data);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
