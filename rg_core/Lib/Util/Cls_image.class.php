...
class Cls_image {
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='', $filename = '')
    {
        ...
        if (!$org_info)
        {
            $this->error_msg = sprintf('没有找到原始文件', $img);
        ...
        {
            $this->error_msg = sprintf('不支持图片类型', $this->type_maping[$org_info[2]]);
     ...
     */
    function add_watermark($filename, $target_file='', $watermark='', $watermark_place='', $watermark_alpha = 0.65, $ptop = 0, $pleft = 0, $w_text='',$w_font=0,$wx=0,$wy=0)
    {
        // 是否安装了GD
        $gd = $this->gd_version();
        if ($gd == 0)
        ...
        {
            return str_replace(APP_PATH, '', str_replace('\\', '/', realpath($filename)));
        }

        if (!$this->validate_image($watermark) && !$w_text)
        {
            /* 已经记录了错误信息 */
            return false;
        }
        ...
        switch ($watermark_place)
        {
            case '1':
                ...
                break;
            case '2':
                $x = $source_info[0] - $watermark_info[0] + $pleft;
                $y = 0 + $ptop;
                break;
            case '4':
                $x = 0 + $pleft;
                $y = $source_info[1] - $watermark_info[1] + $ptop;
                break;
            case '5':
                $x = $source_info[0] - $watermark_info[0] + $pleft;
                $y = $source_info[1] - $watermark_info[1] + $ptop;
                break;
            default:
                $x = $source_info[0] / 2 - $watermark_info[0] / 2 + $pleft;
                $y = $source_info[1] / 2 - $watermark_info[1] / 2 + $ptop;
        }
		
		if($ifwaterimage && !$w_text) {
			...
			$r = hexdec(substr('#666666',1,2));
			$g = hexdec(substr('#666666',3,2));
			$b = hexdec(substr('#666666',5));
 
			$black = imagecolorallocate($source_handle, $r,$g, $b);//设置颜色   
			imagettftext($source_handle, $w_font, 0, $wx, $wy, $black, APP_PATH."../data/Font/hwxh.ttf", $w_text);//打印水印 
		}
        ...
        if ($path)
        {
            return str_replace(APP_PATH, '', str_replace('\\', '/', $path));
        }
        else
        {
            return false;
